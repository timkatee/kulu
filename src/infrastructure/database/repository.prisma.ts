...
export class RepositoryPrisma<T> implements Repository<T> {
    ...
    async create(data: T): Promise<T> {
        // @ts-ignore
        return  prisma[this.entity].create({data}).catch(err => this.onError(err));
    }
    ...
    async update(id: number, data: T): Promise<T> {
        // @ts-ignore
        return  prisma[this.entity].update({where: {id}, data}).catch(err => this.onError(err));
    }
     ...
     * @param params
     */
    async readSingle(id: number | string, metaData: any, params: any): Promise<T> {
        ...
        // @ts-ignore
        return  prisma[this.entity].findUnique({...{where: {id: id}, ...{select: selectFields}, ...whereOverride}}).catch(err => this.onError(err));
    }
    ...
    async readMany(filters: Partial<RepositoryFilter>, metaData: any): Promise<T[]> {
        ...
        // @ts-ignore
        return  prisma[this.entity].findMany({...filters, ...{select: selectFields}}).catch(err => this.onError(err));
    }
    ...
    async delete(id: number): Promise<T> {
        // @ts-ignore
        return  prisma[this.entity].delete({where: {id}}).catch(err => this.onError(err));
    }
